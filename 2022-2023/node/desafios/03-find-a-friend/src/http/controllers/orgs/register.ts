...
export async function register(request: FastifyRequest, reply: FastifyReply) {
  const registerOrgBodySchema = z.object({
    ...
    zip_code: z.string().refine(value => value.length === 8 && /^\d+$/.test(value), {
      message: 'CEP must have 8 numeric digits'
    }),
    address: z.string(),
    whatsapp: z.string().refine(value => value.length === 11 && /^\d+$/.test(value), {
      message: 'CEP must have 11 numeric digits'
    }),
  })
  
  const { responsible_name, email, password, zip_code, address, whatsapp } = registerOrgBodySchema.parse(request.body)
